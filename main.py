...
def self_ping_keepalive(interval_seconds: int = 60):
    """
    High-Frequency External Keepalive Daemon:
    Periodically sends public HTTPS GET requests to the external Hugging Face Space domain (/healthz, /api/system_status)
    to generate active ingress traffic and permanently prevent the Space from pausing.
    """
    time.sleep(20)  # Initial grace period on boot
    
    hf_host = os.environ.get("SPACE_HOST", "abaja-notes-taker.hf.space").strip()
    if not hf_host:
        hf_host = "abaja-notes-taker.hf.space"
    
    hf_base = f"https://{hf_host}" if not hf_host.startswith("http") else hf_host
    print(f"[*] Robust HF Space Keepalive Daemon active. Monitoring: {hf_base} (every {interval_seconds}s)", flush=True)
    ...
    while True:
        # Ping public HF Space endpoints
        for endpoint in ["/healthz", "/api/system_status"]:
            try:
                req = urllib.request.Request(f"{hf_base}{endpoint}", headers=headers)
...
def main():
    ...
    # 4. Start Telegram Bot Poller on Hugging Face (Routed via Render Proxy)
    if os.environ.get("TELEGRAM_BOT_TOKEN"):
        print("[*] Launching Telegram Bot Poller (bot.py) on Hugging Face via Render Proxy Gateway...")
